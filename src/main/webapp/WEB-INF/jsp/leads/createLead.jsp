        ...
        <body>
            ...
            <div class="page-container" style="padding: 30px; max-width: 960px; margin: 0 auto;">
                <div class="page-header"
                    ...
                    <h2 style="color: var(--text-primary); font-size: 1.5rem; font-weight: 700; margin:0;">Add New Lead
                    </h2>
                    <a href="${pageContext.request.contextPath}/view_filter_leads" class="btn-secondary">← Back to
                        Leads</a>
                </div>

                <c:if test="${not empty error}">
                    <div
                        style="background:#fee2e2; color:#991b1b; padding:12px 16px; border-radius:8px; margin-bottom:16px; font-weight:500;">
                        ${error}</div>
                </c:if>

                <div class="card"
                    style="background:rgba(255,255,255,0.95); border-radius:16px; padding:32px; box-shadow:0 4px 24px rgba(0,0,0,0.08);">
                    <form id="leadForm" action="${pageContext.request.contextPath}/create_lead" method="post">
                        ...
                        <%-- ── CLIENT SEARCH ──────────────────────────────────────────────── --%>
                            ...
                            <div style="margin-bottom:20px;">
                                <label class="form-label-text" for="clientSearch">Search Client <span
                                        class="field-required">*</span></label>
                                <div class="search-wrapper">
                                    <input type="text" id="clientSearch" autocomplete="off"
                                        placeholder="Search clients…" class="form-input">
                            ...
                            <%-- Client info preview card (shown after selection) --%>
                                <div id="clientInfoCard" class="client-info-card">
                                    ...
                                    <div class="client-info-grid">
                                        <div class="client-info-item"><label>Name</label><span id="ci_name">—</span>
                                        </div>
                                        <div class="client-info-item"><label>Mobile</label><span id="ci_mobile">—</span>
                                        </div>
                                        <div class="client-info-item"><label>Email</label><span id="ci_email">—</span>
                                        </div>
                                        <div class="client-info-item"><label>City</label><span id="ci_city">—</span>
                                        </div>
                                        <div class="client-info-item"><label>Country</label><span
                                                id="ci_country">—</span></div>
                                        <div class="client-info-item"><label>Organisation</label><span
                                                id="ci_org">—</span></div>
                                        <div class="client-info-item"><label>Org. Type</label><span
                                                id="ci_orgType">—</span></div>
                                        <div class="client-info-item"><label>Reg. Number</label><span
                                                id="ci_regNum">—</span></div>
                                        <div class="client-info-item"><label>Designation</label><span
                                                id="ci_designation">—</span></div>
                                        <div class="client-info-item"><label>Client Type</label><span
                                                id="ci_clientType">—</span></div>
                                        <div class="client-info-item"><label>Client Source</label><span
                                                id="ci_clientSource">—</span></div>
                                        <div class="client-info-item"><label>Website</label><span
                                                id="ci_website">—</span></div>
                                        <div class="client-info-item"><label>Address</label><span
                                                id="ci_address">—</span></div>
                                        <div class="client-info-item"><label>Postal Code</label><span
                                                id="ci_postal">—</span></div>
                                    </div>
                                    <button type="button" id="clearClientBtn"
                                        style="margin-top:12px; font-size:12px; background:none; border:1px solid #cbd5e1; border-radius:6px; padding:4px 12px; cursor:pointer; color:#64748b;">✕
                                ...
                                <%-- ── LEAD DETAILS ──────────────────────────────────────────────── --%>
                                    ...
                                    <div
                                        ...
                                        <div>
                                            <label class="form-label-text" for="leadTitle">Lead Title <span
                                                    class="field-required">*</span></label>
                                            <input type="text" id="leadTitle" name="leadTitle" class="form-input"
                                                value="${LEAD_OBJ.leadTitle}">
                                        </div>
                                        <div>
                                            <label class="form-label-text" for="leadName">Lead Name</label>
                                            <input type="text" id="leadName" name="leadName" class="form-input"
                                                value="${LEAD_OBJ.leadName}">
                                        </div>
                                    </div>
                                    ...
                                    <%-- ── LEAD CLASSIFICATION ──────────────────────────────────────── --%>
                                        ...
                                        <div
                                            ...
                                            <div>
                                                <label class="form-label-text" for="leadStatus">Status <span
                                                        class="field-required">*</span></label>
                                                <select id="leadStatus" name="leadStatus" class="form-select">
                                                    <option value="">-- Select Status --</option>
                                                    <c:forEach var="s" items="${LEAD_STATUSES}">
                                                        <option value="${s}" <c:if test="${LEAD_OBJ.leadStatus == s}">
                                                            selected</c:if>>${s}</option>
                                                    </c:forEach>
                                                </select>
                                            </div>
                                            <div>
                                                ...
                                                <select id="priority" name="priority" class="form-select">
                                                    ...
                                                    <c:forEach var="p" items="${PRIORITIES}">
                                                        <option value="${p}" <c:if test="${LEAD_OBJ.priority == p}">
                                                            selected</c:if>>${p}</option>
                                                    </c:forEach>
                                                </select>
                                            </div>
                                            <div>
                                                <label class="form-label-text" for="leadSource">Lead Source <span
                                                        class="field-required">*</span></label>
                                                <select id="leadSource" name="leadSource" class="form-select">
                                                    <option value="">-- Select Source --</option>
                                                    <c:forEach var="src" items="${CLIENT_SOURCES}">
                                                        <option value="${src.sourceName}" <c:if
                                                            test="${LEAD_OBJ.leadSource == src.sourceName}">selected
                                                            </c:if>>${src.sourceName}</option>
                                                    </c:forEach>
                                                </select>
                                            </div>
                                            <c:choose>
                                                <c:when
                                                    test="${CURRENT_USER.hasRole('ADMIN') || CURRENT_USER.hasRole('SUPERADMIN')}">
                                                    <div>
                                                        <label class="form-label-text" for="assignedTo">Assigned To
                                                        ...
                                                        <select id="assignedTo" name="assignedTo" class="form-select">
                                                            ...
                                                            <c:forEach var="entry" items="${ACTIVE_USERS_MAP}">
                                                                <option value="${entry.key}" <c:if
                                                                    test="${LEAD_OBJ.assignedTo == entry.key}">selected
                                        ...
                                        <%-- ── NOTIFICATION CONTROLS ────────────────────────────────────── --%>
                                            <div class="section-title" style="margin-top:24px;">Notification Settings
                                            </div>
                                            <div
                                                style="display:flex; gap:24px; margin-bottom:24px; background: rgba(99,102,241,0.03); border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px 20px;">
                                                <label
                                                    style="display:flex; align-items:center; gap:8px; font-size:0.9rem; font-weight:600; color:#475569; cursor:pointer;">
                                                    <input type="checkbox" name="notifyEmail" id="notifyEmail"
                                                        style="width:18px; height:18px; cursor:pointer;">
                                                    Notify Client via Email
                                                </label>
                                                <label
                                                    style="display:flex; align-items:center; gap:8px; font-size:0.9rem; font-weight:600; color:#475569; cursor:pointer;">
                                                    <input type="checkbox" name="notifyWhatsApp" id="notifyWhatsApp"
                                                        style="width:18px; height:18px; cursor:pointer;">
                                            ...
                                            <%-- ── ACTIONS ───────────────────────────────────────────────────── --%>
                                                <div
                                                    ...
                                                    <button type="submit"
                                                        style="padding:10px 28px; background: linear-gradient(135deg,#6366f1,#4f46e5); color:#fff; border:none; border-radius:8px; cursor:pointer; font-weight:700; font-size:15px; box-shadow:0 4px 12px rgba(99,102,241,0.3);">Create
                                                        Lead</button>
                                                </div>
                    </form>
                </div>
